...
The home page is `/`. A season's week results live at `/:season/:week/scoreboard` and `/:season/:week/picks`, so they can be linked and reloaded, and `/scoreboard` and `/picks` redirect to the latest week worth showing. Picks come from `/api/picks/<year>/<week>` first, then from a cache of anything uploaded in that browser for that season and week. A week with no picks either way sends you home with an explanation. Requires Node `v22` (`.nvmrc`). Run `nvm use` first.
