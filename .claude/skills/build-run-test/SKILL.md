...
- Base UI for behavior, SCSS for looks. Base UI ships no styles, so every visual lives in SCSS against the `--rak-*` tokens in `src/index.scss`. There is no theme provider. `@mui/icons-material` is for icons only.
...
153 cases, 13 suites, all offline. `npm test` is `vitest run`; `npm run test:watch` watches.
