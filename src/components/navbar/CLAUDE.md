...
away once the week is decided, since rescoring cannot change a finished week. They
fade and narrow out over `COLLAPSE_DURATION_MS` and then unmount, and cancel
`Navbar`'s `--navbar-gap` with a negative margin so no gap is left where they were.
