 ...
 * is the week that was actually tried. That is what `settledWeek` is for.
 */
export default function useWeekRouteGuard(rawWeek?: string): GuardResult {
  ...
  // The URL is the source of truth for which week is showing. Comparing before
  // writing matters: setting the same week again would restart the picks fetch
  // and score it twice.
  useEffect(() => {
  ...
  if (isWeekInfoLoading) {
    ...
    redirect = {
      header: "Unknown Week",
      message: `Week ${rawWeek} isn't part of the current NFL season, so there is nothing to show.`,
    };
    ...
    redirect = {
      header: "No Results",
      message: `There are no results for week ${weekNumber} yet. Upload a picks spreadsheet to see them.`,
    };
  } else {
